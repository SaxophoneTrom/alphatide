...
def scenario_events() -> list[TransferEvent]:
    # Wintermute AND Jump both accumulate mETH → convergence (B).
    # Binance (cex) pushes WMNT into a fresh wallet → inflow (C).
    return [
...
def run_scenario() -> None:
    ...
    # seed a realistic (varied) volume baseline so the anomaly detector fires
    history = {
    ...
    print("-" * 64)
    print(f"\nSurf credits used: {surf.credits_used} "
          f"(offline fixtures = 0; live = 1 shared batch for ALL detectors)")
